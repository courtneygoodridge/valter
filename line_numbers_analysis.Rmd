...
In the lie condition, sometimes the picture is the lawyer and other times it is the cop. When it is the cop, they have to lie (i.e. respond with the opposite key, if the number is odd they had to respond as if it is  even). When the picture is a lawyer, they have to tell the truth (i.e. respond with the correct key). Here 2/3 trials are lawyer (standard SNARC) whereas the 1/3 of trials participants had to lie (does this have any effect on the SNARC?)
...
MARC effect - Markedness of Response Codes effect proposes faster reaciton times for left/right hand side responses to odd/even numbers
...
2) When you have a dichotomous dependent variable, it is desirable to have a model and that predicts whether a value is 0 or 1. A linear regression (essentially a standard ANOVA) does not allow you to do this because the predicted y variable may not be restricted to a value between 0 and 1. Conversely, a logistic model can produce the odds of the occurrence of an event. 
...
## computing inverse efficiency scores

```{r}
correct_response_rates <- workingdata %>%
  dplyr::filter(condition == "controltrue") %>%
...
controltrue_dat <- cbind(correct_response_rates, correct_RTs)

controltrue_dat <- controltrue_dat %>%
  dplyr::group_by(participant) %>%
  dplyr::mutate(IES = meanRT / correct_RR)
  
ggplot(controltrue_dat, aes(x = correct_RR, y = meanRT)) +
  geom_point() +
  ylim(0, 1.5)

"ANOVA on IES metric for SNARC effect"
result_IES <- ezANOVA(controltrue_dat, dv = IES, wid = participant, within = .(response_hand, magnitude), type = 3, detailed = TRUE, return_aov = TRUE)

result_IES

ggplot(controltrue_dat %>%
...
"ANOVA on RT for SNARC effect"
result_RT <- ezANOVA(workingdata %>%
                       dplyr::filter(condition == "controltrue") %>%
                       dplyr::filter(accuracy == 1), dv = RTs, wid = participant, within = .(response_hand, magnitude), type = 3, detailed = TRUE, return_aov = TRUE)

result_RT

"ANOVA on errors for SNARC effect"
result_errors <- ezANOVA(workingdata %>%
                           dplyr::filter(condition == "controltrue") %>%
                           dplyr::filter(accuracy == 1) %>%
                           dplyr::group_by(participant, response_hand, magnitude) %>%
                           dplyr::summarise(errors = n()) %>%
                           drop_na, dv = errors, wid = participant, within = .(response_hand, magnitude), type = 3, detailed = TRUE, return_aov = TRUE)
