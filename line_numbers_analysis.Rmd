...
result_MARC <- ezANOVA(workingdata %>%
                         drop_na() %>%
                         dplyr::filter(condition == "exptrue"), dv = RTs, wid = participant, within = .(parity, response_hand), type = 3, detailed = TRUE, return_aov = TRUE)
result_MARC
...
T-test reveales significant odd effect - faster reaction times for even numbers, however the effect is smaller.
 
**MARC effect**

ANOVA reveals significant interaction between parity and hand responded with, alongside a large effect size indicating a MARC effect (however I would still consider a GLM due to distribution of RT data). 
...
result_SNARC <- ezANOVA(workingdata %>%
                          drop_na() %>%
                          dplyr::filter(condition == "exptrue"), dv = RTs, wid = participant, within = .(magnitude, response_hand), type = 3, detailed = TRUE, return_aov = TRUE)
result_SNARC
...
A significant interaction with large effect size indicating a classic SNARC in the exerimental condition where a lawyer is present. When responding to larger numbers with right hard, RTs are faster. When responding to small numbers with left hand, RTs are faster). 
...
Odds ratio of the *parityeven* object estimate reveals that odds of an accuarate response when the number is even are 1.3:1, suggesting they are more likely. 
...
We focus on the significant interaction between parity and hand via the *handright:parityeven* object. The odds ratio indicates odds of an accurate response are when hand and parity are congruant arre 0.55:1, suggesting that errors are more likely. Similar to the control condiition, it appears their is an inversed MARC effect. This is visualised in via the line plot.
...
The odd ratio of the *handleft:magnitudelarge* interaction object suggests that when responding with incongruant hand and number magnitude, the odds of an accurate response increase to 1.72:1. suggesting accuracy in responses increases. Once again, this is the opposite of the traditional SNARC effect.
...
result_MARC <- ezANOVA(workingdata %>%
                         drop_na() %>%
                         dplyr::filter(condition == "explie"), dv = RTs, wid = participant, within = .(parity, response_hand), type = 3, detailed = TRUE, return_aov = TRUE)
result_MARC
...
ANOVA reveals non-significant main effects and interactions. Line plot shows that reverse MARC effect could be happening however this is not significant.
...
result_SNARC <- ezANOVA(workingdata %>%
                          drop_na() %>%
                          dplyr::filter(condition == "explie"), dv = RTs, wid = participant, within = .(magnitude, response_hand), type = 3, detailed = TRUE, return_aov = TRUE)
result_SNARC
...
Odds ratio of the *parityeven* is 1.10, suggesting that an accurate response is no more likely when presented with an even number of an odd number. This demonstrates no odd effect.
...
Line plot shows evidence of an interaction with a characteristic MARC effect. The interaction from the logistic model shows that the interaction is small, and just misses out on being significant. Looking at the odds ratios, the odds of an accurate response when the response hand and parity are equal (*response_handright:parityeven* object) is 0.76:1. This is not far off 1:1 which would equate to equal chances for accurate and error response. This explains why the interaction just misses out on being significant.

This finding is important, because a 2x2 ANOVA reveals a significant effect, however I do not believe it is the appropriate analysis. This is something to think about. 
...
The odd ratio of the *handleft:magnitudelarge* interaction object suggests that when responding with incongruant hand and number magnitude, the odds of an accurate response increase to 0.94:1, suggesting that when response hands and magnitude are incongruent, there is equal probability of an accurate or error response. 
