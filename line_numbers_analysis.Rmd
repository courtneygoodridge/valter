...
CI_OddEffect <- summarySE(lawyeronly, measurevar = "RTs", groupvars = c("parity"))
...
ggplot(lawyeronly %>%
         ...
         dplyr::filter(accuracy == 0), aes(x = parity, y = acc)) +
  geom_col() +
  ylab("Numbers of errors") +
...
To further investiagte this, we must look at the estimate. The estimate of logistic regression refers to the "log odds". To reveal the probability, I exponeniate the estimate. This indicates that the odds of a correct response increase by a factor of 1.66946 (66%) when numbers are even. Hence this statistically highlights the odd effect. 
...
**Notes on correlated random effects**

(1 | x) -> Random intercept for each value of x

(1 + a | x) -> Random intercept for each value of x and random slope dependent on a. lme4 assumes that intercept and coeffecient correlated however this isn't always the case.

(1 + a || x) -> uncorrelated random effects (investigate why this makes a difference)
...
- plot logistic glmers 
- investigate logistic glmers with more than one predictor (only necessary if I include random slope model)
- sort out odd effect t-test for RTs (one subject has a missing observation, might be causing issues)
