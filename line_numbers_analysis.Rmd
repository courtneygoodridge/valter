...
```{r}
ggplot(lawyeronly %>%
         dplyr::group_by(parity, accuracy) %>%
         dplyr::summarise(acc_num = n()) %>%
         dplyr::ungroup() %>%
         dplyr::group_by(parity) %>%
         dplyr::mutate(perc = (acc_num / sum(acc_num)) * 100) %>%
         dplyr::filter(accuracy == 0), aes(x = parity, y = acc_num)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = acc_num, vjust = -1)) +
  ylim(0, 100) +
  ylab("Numbers of errors") +
  scale_x_discrete("Parity", labels = c("odd" = "Odd", "even" = "Even")) +
  ggtitle("Number of errors per parity (Odd effect)") +
  theme_gray() 
...
CIerror_bar_MARC <- summarySE(lawyeronly, measurevar = "accuracy", groupvars = c("parity", "hand"))

ggplot(data = CIerror_bar_MARC, aes(x = parity, color = hand, group = hand, y = accuracy)) +
         stat_summary(fun.y = mean, geom = "point") +
         stat_summary(fun.y = mean, geom = "line") +
...
To perform the error analysis, I compute a multiple logistic regression. Because accuracy is a dichotomous dependent variable,the assumptions of a standard ANOVA will be violated. Hence a logistic regression can be used to model an event that exists in pass/fail, correct/error state, such as accuracy in this case.

First I set up my factors for hand and parity. I then  run a generalised linear model and specify the family as binomial. This allows me to compute the logistic regression

The results of the logistic regression demonstrate a significant interaction. The object *handleft:parityodd* highlights which of the parity/hand pairs is driving the interaction, whereby the probability of an accurate response is significantly reduced.

The line plot visualises this interaction, whereby accuracy is reduced for left + odd pairs and right + even pairs. This is the opposite of the traditional MARC effect. 

**TO DO** 

Compute t test for errors for odd and even numbers.