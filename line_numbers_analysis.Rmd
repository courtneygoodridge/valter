...
A standard ANOVA assumes a normal distribution however RTs rarely follow a normal distribution, hence GLMs often provide more statistical power when analysing RT data because you can specify an error distribution that fits you data (such as Gamma or Inverse-Gaussian). This paper details differences in analysing RT data between linear and generalised linear *mixed effects* model however I think the same applies when not specifying mixed effects (https://www.frontiersin.org/articles/10.3389/fpsyg.2015.01171/full)
